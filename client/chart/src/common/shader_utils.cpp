...
//                    "#define highp  \n"
    ...
    glGetShaderiv(res, GL_COMPILE_STATUS, &Result);
    glGetShaderiv(res, GL_INFO_LOG_LENGTH, &logLength);
    if ( logLength > 0 ){
        std::vector<char> FragmentShaderErrorMessage(logLength+1);
        glGetShaderInfoLog(res, logLength, NULL, &FragmentShaderErrorMessage[0]);
...
GLuint
create_gs_program(const char *vertexfile, const char *geometryfile, const char *fragmentfile, GLint input, GLint output,
                  GLint vertices) {
    GLuint program = glCreateProgram();
    GLuint shader;
...
GLint get_attrib(GLuint program, const char *name) {
    ...
    if (attribute == -1) {
        string logText = "Could not bind attribute ";
        logText += name;
        consoleLog(logText);
    }
    return attribute;
...
GLint get_uniform(GLuint program, const char *name) {
    ...
    if (uniform == -1) {
        string logText = "Could not bind uniform ";
        logText += name;
        consoleLog(logText);
    }
