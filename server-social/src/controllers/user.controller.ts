...
export const userController = {
	...
	async get (reqUser, userId, type: number = USER_FETCH_TYPE_SLIM, forceReload = false) {
		if (!userId)
	...
	async update(id, params) {
		...
		// Update DB
		const user = await User.findAndModify({_id: Types.ObjectId(id)}, params);
		const REDIS_KEY = REDIS_USER_PREFIX + user._id;
