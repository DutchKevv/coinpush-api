...
class Mapper {
    ...
    update(symbol, timeFrame, from, until, count) {
        ...
        map[symbol][timeFrame] = util_date_1.mergeRanges(ranges);
        // Persistent mode
        if (this.mode === Mapper.MODE_PERSISTENT) {
            fs.writeFileSync(this._pathFile, JSON.stringify(map, null, 2));
        }
    }
    isComplete(symbol, timeFrame, from, until, count) {
        if (!symbol)
        ...
        let ranges = this.findByParams(symbol, timeFrame), i = 0, len = ranges.length, _range;
    ...
    getPercentageComplete(symbol, timeFrame, from, until, count) {
        let ranges = this.findByParams(symbol, timeFrame, true), totalRequiredTime = from && until ? until - from : util_date_1.getEstimatedTimeFromCount(timeFrame, count), totalStoredTime = 0, totalCount = 0, result = 0;
        ranges.forEach(range => {
            let _from = range[0], _until = range[1], _count = range[2];
    ...
    getMissingChunks(symbol, timeFrame, from, until, count) {
        let ranges = this.findByParams(symbol, timeFrame, true), result = [{ from, until, count }];
        ranges.forEach(range => {
            if (from && until) {
    ...
    findByParams(symbol, timeFrame, create = true) {
        let map = this.map;
        if (map[symbol])
